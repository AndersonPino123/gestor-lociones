...
def mostrar_registro_compra():
    ...
    lista_productos = [
        f"{fila['id']} - {fila['marca']} | {fila['nombre_producto']} ({fila['genero'].capitalize()})"
        for _, fila in productos.iterrows()
    ]
    seleccion_producto = st.selectbox(
        "Selecciona el producto comprado:", lista_productos
    )
    producto_nombre = seleccion_producto.split(" - ", 1)[1]  # "Marca | Nombre (Género)"
